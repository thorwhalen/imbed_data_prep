...
class TrumpZelenskyyDacc:
    ...
    def __post_init__(self):
        ...
        os.makedirs(
            os.path.join(self.rootdir, os.path.dirname(self.transcript_save_key)),
            exist_ok=True,
        )
        os.makedirs(
            os.path.join(self.rootdir, os.path.dirname(self.embeddings_save_key)),
            exist_ok=True,
        )
    ...
    def process_transcript(self):
        ...
        df["speaker"] = df["speaker"].apply(
            lambda speaker: (
                speaker if speaker in top_speakers else self.other_speaker_label
            )
        )
    ...
    def compute_embeddings(self):
        ...
        if self.embeddings_provider is None:
            try:
                ...
                raise ImportError(
                    "No embeddings provider specified and 'oa' package not available. "
    ...
    def compute_tsne_projection(self):
        ...
        tsne = TSNE(
            n_components=self.tsne_components, random_state=self.tsne_random_state
        )
        tsne_vectors = tsne.fit_transform(embeddings)

        return pd.DataFrame(
            tsne_vectors, columns=[f"tsne_{i + 1}" for i in range(self.tsne_components)]
        )
    ...
    def compute_lda_projection(self):
        ...
        return pd.DataFrame(
            lda_vectors, columns=[f"lda_{i + 1}" for i in range(self.lda_components)]
        )
    ...
    def compute_pca_projection(self):
        ...
        return pd.DataFrame(
            pca_vectors, columns=[f"pca_{i + 1}" for i in range(self.pca_components)]
        )
    ...
    def get_speaker_data(self, speaker=None):
        ...
        if speaker not in df["speaker"].unique():
            raise ValueError(
                f"Speaker '{speaker}' not found. Available speakers: {df['speaker'].unique().tolist()}"
            )
    ...
    def get_sentiment_summary(self, by_speaker=True):
        ...
        if "compound" not in df.columns:
            raise ValueError(
                "Sentiment analysis not available in the complete DataFrame."
            )
